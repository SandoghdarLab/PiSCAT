...
class BatchAnalysis(QtWidgets.QMainWindow):
    def __init__(self, object_update_progressBar, parent=None):
        ...
        self.hyperparameters = {
            ...
            "max_sigma": None,
            "sigma_ratio": None,
            "PSF_detection_thr": None,
            "overlap": None,
            "search_range": None,
            "memory": None,
            "outlier_frames_thr": None,
            "symmetric_PSFs_thr": None,
            "min_V_shape_width": None,
        }
        ...
        self.flags = {
            ...
            "DRA": None,
            "FPNc": None,
            "outlier_frames_filter": None,
            "Dense_Filter": None,
            "symmetric_PSFs_Filter": None,
            "FFT_flag": None,
        }
    ...
    def initUI(self):
        ...
        self.all_tabs["PSFs_Localization"].output_setting_Tab_Localization_external.connect(
            self.Update_tab_localization
        )
    ...
    def update_tab(self, idx_):
        if idx_ == 4:
            self.all_tabs["Protein_Analysis"].updata_input_setting(
                [self.hyperparameters, self.flags]
            )
    ...
    def Update_tab_localization(self, data_in):
        for key in data_in:
            if key == "mode":
                ...
                key == "Side_lobes_Filter"
                or key == "Dense_Filter"
                or key == "outlier_frames_filter"
                or key == "symmetric_PSFs_Filter"
            ):
                self.flags[key] = data_in[key]
            else:
                self.hyperparameters[key] = data_in[key]
