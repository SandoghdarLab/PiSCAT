        ...
        if flag_run:
            if self.input_video is None:
                ...
                if self.empty_value_box_flag:
                    ...
                    self.df_PSFs_preview = psf.psf_detection_preview(
                        function=self.combo_item,
                    ...
                    if self.groupBox_fine_localization.isChecked():
                        self.df_PSFs_preview = psf.improve_localization_with_frst(
                            df_PSFs=self.df_PSFs_preview, scale=self.win_size, flag_preview=True
                        )
    ...
    def do_localization(self):
        ...
        if flag_run:
            ...
            if self.empty_value_box_flag:
                ...
                self.df_PSFs = psf.psf_detection(
                    function=self.combo_item,
                    min_sigma=self.min_sigma,
                    max_sigma=self.max_sigma,
                    sigma_ratio=self.sigma_ratio,
            ...
            if self.df_PSFs is not None:
                ...
                if self.empty_value_box_flag:
                    psf = particle_localization.PSFsExtraction(
                        video=self.input_video, flag_GUI=True
                    )
                    self.df_PSFs = psf.improve_localization_with_frst(
                        df_PSFs=self.df_PSFs, scale=self.win_size, flag_preview=False
                    )
    ...
    def do_Filtering(self):
        if self.input_video is None:
            ...
            if self.checkbox_crappy_frames.isChecked():
                ...
                if self.empty_value_box_flag:
                    self.df_PSFs_s_filter = s_filters.outlier_frames(
                        self.df_PSFs, threshold=self.crappy_thr
                    )
                    ...
                    self.PSFs_Particels_num[
                        "#PSFs_after_outlier_frames_filtering"
            ...
            if self.checkbox_filter_double_PSF.isChecked():
                ...
                self.PSFs_Particels_num[
                    "#PSFs_after_dense_PSFs_filtering"
                ] = self.df_PSFs_s_filter.shape[0]

            if self.checkbox_filter_side_lobes_PSF.isChecked():
                self.df_PSFs_s_filter = s_filters.remove_side_lobes_artifact(
                    self.df_PSFs_s_filter, threshold=0
                )
                self.setting_localization["Flag_filter_side_lobes_PSFs"] = True
                self.PSFs_Particels_num[
                    "#PSFs_after_side_lobes_PSFs_filtering"
                ] = self.df_PSFs_s_filter.shape[0]

            if (
                self.checkbox_2DFitting.isChecked()
                and self.checkbox_filter_asymmetry_PSF.isChecked()
            ):
                pass
            ...
            if self.checkbox_2DFitting.isChecked():
                ...
                if self.empty_value_box_flag:
                    psf_localization = particle_localization.PSFsExtraction(
                        video=self.input_video
                    )
                    self.df_PSFs_s_filter = psf_localization.fit_Gaussian2D_wrapper(
                        PSF_List=self.df_PSFs_s_filter,
            ...
            if (
                self.checkbox_filter_asymmetry_PSF.isChecked()
                and self.checkbox_2DFitting.isChecked()
            ):
                self.get_values_asymmetry_filtering()
                if self.empty_value_box_flag:
                    self.df_PSFs_s_filter = s_filters.symmetric_PSFs(
                        self.df_PSFs_s_filter, threshold=self.thr_sigma
                    )

                    self.setting_localization["Flag_filter_asymmetry_PSFs"] = True
                    self.setting_localization[
                        "Asymmetry_PSFs_filtering_threshold"
                    ] = self.thr_sigma
                    self.setting_localization[
                        "Asymmetry_PSFs_filtering_win_size (px)"
                    ] = self.scale
                    self.PSFs_Particels_num[
