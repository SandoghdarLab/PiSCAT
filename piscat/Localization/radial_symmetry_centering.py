...
class RadialCenter:
    ...
    def patch_genrator(self, original_video, patch_size=16, strides=4):
        ...
        self.patch = self.patch_gen.split_weight_matrix(
            original_video, patch_size=patch_size, strides=strides
        )
    ...
    def radialcenter_probility_map(self):
        if self.patch is not None and self.patch_gen is not None and self.patch_gen is not None:
            ...
            self.probility_map = self.patch_gen.reconstruction_weight_matrix(
                self.video_shape, new_patch=patch_probility
            )
