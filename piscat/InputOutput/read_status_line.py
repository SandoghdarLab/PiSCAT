...
class StatusLine:
    ...
    def read_status_line(self, frame):
        ...
        if width >= 12:
            RealTimeCnt = (
                sum(np.multiply(StatusLine[8:12], converter)) / NrAcc
            )  # (Mean) Real Time Counter in Microseconds
            self.camera_info["RealTimeCnt"] = RealTimeCnt

        if width >= 16:
            MissedTrgCnt = (
                sum(np.multiply(StatusLine[12:16], converter)) / NrAcc
            )  # (Mean) Missed Trigger Counter
            self.camera_info["MissedTrgCnt"] = MissedTrgCnt

        if width >= 20:
            self.ImgAvgVal = (
                sum(np.multiply(StatusLine[16:20], converter)) / NrAcc
            )  # (Mean) Image Average Value
            self.camera_info["RealTimeCnt"] = RealTimeCnt

        if width >= 24:
            IntTimeClk = (
                sum(np.multiply(StatusLine[20:24], converter)) / NrAcc
            )  # Integration Time in units of clock cycles (80MHz)
        ...
        if width >= 28:
            BrstTrgNr = (
                sum(np.multiply(StatusLine[24:28], converter)) / NrAcc
            )  # (Mean) Burst Trigger Number
            self.camera_info["BrstTrgNr"] = BrstTrgNr

        if width >= 32:
            MissedBrstTrgCnt = (
                sum(np.multiply(StatusLine[28:32], converter)) / NrAcc
            )  # (Mean) Missed Burst Trigger Counter
            self.camera_info["MissedBrstTrgCnt"] = MissedBrstTrgCnt

        if width >= 36:
            ROIx_start = (
                sum(np.multiply(StatusLine[32:36], converter)) / NrAcc
            )  # Horizontal start position of ROI (Window.X)
            self.camera_info["ROIx_start"] = ROIx_start

        if width >= 40:
            ROIx_end = (
                sum(np.multiply(StatusLine[36:40], converter)) / NrAcc
            )  # Horizontal end position of ROI (= Window.X + Window.W - 1)
        ...
        if width >= 52:
            TrgSrc = (
                sum(np.multiply(StatusLine[48:52], converter)) / NrAcc
            )  # Trigger Source (0=Free Running, 1=Interface Trigger, 2=I/O Trigger)
            self.camera_info["TrgSrc"] = TrgSrc

        if width >= 56:
            DigGain = 2 ** (
                sum(np.multiply(StatusLine[52:56], converter)) / NrAcc
            )  # Digital Gain (1x, 2x, 4x, 8x)
        ...
        if width >= 64:
            CamType = (
                sum(np.multiply(StatusLine[60:64], converter)) / NrAcc
            )  # Camera Type Code (Our Photonfocus "MV1-D1024E-160-CL-12" is 110)
            self.camera_info["CamType"] = CamType

        if width >= 68:
            SerialNr = (
                sum(np.multiply(StatusLine[64:68], converter)) / NrAcc
