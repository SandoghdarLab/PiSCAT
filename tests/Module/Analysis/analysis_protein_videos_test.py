...
class ProteinAnalysisTest(unittest.TestCase):
    def setUp(self):
        ...
        self.hyperparameters = {
            ...
            "batch_size": 10,
            "min_V_shape_width": 3,
            "max_V_shape_width": 30,
            "search_range": 2,
            "memory": 2,
            "min_sigma": 1.6,
            "max_sigma": 1.8,
            "sigma_ratio": 1.1,
            "PSF_detection_thr": 8e-4,
            "overlap": 0,
            "outlier_frames_thr": 20,
            "Mode_PSF_Segmentation": "BOTH",
            "symmetric_PSFs_thr": 0.7,
            "mode_FPN": "mFPN",
        ...
        self.flag = {
            "PN": True,
            "FPNc": True,
            "outlier_frames_filter": True,
            "Dense_Filter": True,
            "symmetric_PSFs_Filter": True,
            "FFT_flag": False,
            "filter_hotPixels": True,
        }

    def test_protein_analysis(self):
        protein_analysis(
            self.directory_path, self.file_name, self.hyperparameters, self.flag, "mFPN"
        )
        df_histogram = reading_videos.DirectoryType(
            os.path.join(self.directory_path[0], "mFPN"), type_file="json"
        ).return_df()
        ...
        for path, file_name in zip(paths, file_names):
            if "hyperparameters" in file_name:
                ...
                particles_loaded = read_write_data.read_json2dic(
                    self.directory_path[0], "PSFs_Particels_num.json"
                )
        ...
        df_histogram = reading_videos.DirectoryType(
            os.path.join(self.directory_path[0], "mFPN"), type_file="mat"
        ).return_df()
        paths = df_histogram["Directory"].tolist()
        file_names = df_histogram["File"].tolist()
        for path, file_name in zip(paths, file_names):
            if "all_trajectories" in file_name:
                ...
                loaded_all_trajectories = read_write_data.read_mat(
                    self.directory_path[0], "all_trajectories.mat"
                )
                self.assertTrue(check_trajectory(all_trajectories, loaded_all_trajectories))

        df_histogram = reading_videos.DirectoryType(
            os.path.join(self.directory_path[0], "mFPN"), type_file="csv"
        ).return_df()
        paths = df_histogram["Directory"].tolist()
        file_names = df_histogram["File"].tolist()
        for path, file_name in zip(paths, file_names):
