...
class TestDifferentialRollingAverage(unittest.TestCase):
    ...
    def test_differential_rolling(self):
        ...
        expected_shape = (self.test_obj.video.shape[0] - 2 * self.test_obj.batchSize,
    ...
    def test_differential_rolling_mFPN_row(self):
        self.test_obj.mode_FPN = 'mFPN'
        output_video, _ = self.test_obj.differential_rolling(FPN_flag=True, select_correction_axis=0)
        expected_shape = (self.test_obj.video.shape[0] - 2 * self.test_obj.batchSize,
                          self.test_obj.video.shape[1], self.test_obj.video.shape[2])
    ...
    def test_differential_rolling_mFPN_both(self):
        self.test_obj.mode_FPN = 'mFPN'
        output_video, _ = self.test_obj.differential_rolling(FPN_flag=True, select_correction_axis='Both')
        expected_shape = (self.test_obj.video.shape[0] - 2 * self.test_obj.batchSize,
                          self.test_obj.video.shape[1], self.test_obj.video.shape[2])
