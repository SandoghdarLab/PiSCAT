...
from piscat.Localization import difference_of_gaussian
import unittest
import os
import pickle
...
def psf_detection_preview(psf_obj, function_name: str, frame_number, mode):
    psf_dataframe_list_of_frame = psf_obj.psf_detection_preview(function=function_name,
                                                                min_sigma=1.6, max_sigma=1.8, sigma_ratio=1.1,
                                                                threshold=8.5e-4,
                                                                overlap=0, mode=mode, frame_number=frame_number)
    return psf_dataframe_list_of_frame


def psf_detection(psf_obj, function_name: str, mode):
    psf_dataframe_list_of_frame = psf_obj.psf_detection(function=function_name,
                                                        min_sigma=1.6, max_sigma=1.8, sigma_ratio=1.1,
                                                        threshold=8.5e-4,
                                                        overlap=0, mode=mode)
    return psf_dataframe_list_of_frame
...
class TestPSFsExtraction(unittest.TestCase):
    def setUp(self):
        ...
        file_name_save = os.path.join(self.directory_path, 'test_localization_input_video_doh.pck')
    ...
    def test_psf_detection_preview_dog_video_BOTH(self):
        psf_dataframe_list_of_frame = psf_detection_preview(self.test_obj, 'dog', frame_number=[0], mode='BOTH')
        psf_dataframe_one_frame = psf_detection_preview(self.test_obj, 'dog', frame_number=0, mode='BOTH')
        ...
        loaded_data = load_fixture(os.path.join(self.directory_path, 'test_psf_detection_preview_dog_video_both.pck'))
    ...
    def test_psf_detection_preview_dog_video_Bright(self):
        psf_dataframe_list_of_frame = psf_detection_preview(self.test_obj, 'dog', frame_number=[0], mode='Bright')
        psf_dataframe_one_frame = psf_detection_preview(self.test_obj, 'dog', frame_number=0, mode='Bright')
        self.assertTrue(psf_dataframe_list_of_frame.shape == psf_dataframe_one_frame.shape)
        self.assertTrue(psf_dataframe_list_of_frame.equals(psf_dataframe_list_of_frame))
        loaded_data = load_fixture(os.path.join(self.directory_path, 'test_psf_detection_preview_dog_video_Bright.pck'))
        self.assertTrue(np.all((psf_dataframe_one_frame - loaded_data) < 1e-6))
        self.assertTrue(np.all(np.nan_to_num(psf_dataframe_list_of_frame - loaded_data) < 1e-6))

    def test_psf_detection_preview_dog_video_Dark(self):
        psf_dataframe_list_of_frame = psf_detection_preview(self.test_obj, 'dog', frame_number=[0], mode='Dark')
        psf_dataframe_one_frame = psf_detection_preview(self.test_obj, 'dog', frame_number=0, mode='Dark')
        self.assertTrue(psf_dataframe_list_of_frame.shape == psf_dataframe_one_frame.shape)
        self.assertTrue(psf_dataframe_list_of_frame.equals(psf_dataframe_list_of_frame))
        loaded_data = load_fixture(os.path.join(self.directory_path, 'test_psf_detection_preview_dog_video_Dark.pck'))
        self.assertTrue(np.all((psf_dataframe_one_frame - loaded_data) < 1e-6))
        self.assertTrue(np.all(np.nan_to_num(psf_dataframe_list_of_frame - loaded_data) < 1e-6))

    def test_psf_detection_preview_doh_video(self):
        psf_dataframe_list_of_frame = psf_detection_preview(self.test_obj_doh, 'doh', frame_number=[0], mode='BOTH')
        psf_dataframe_one_frame = psf_detection_preview(self.test_obj_doh, 'doh', frame_number=0, mode='BOTH')
        self.assertTrue(psf_dataframe_list_of_frame.shape == psf_dataframe_one_frame.shape)
        self.assertTrue(psf_dataframe_list_of_frame.equals(psf_dataframe_list_of_frame))
        loaded_data = load_fixture(os.path.join(self.directory_path, 'test_psf_detection_preview_doh_video.pck'))
        self.assertTrue(np.all((psf_dataframe_one_frame - loaded_data) < 1e-6))
        self.assertTrue(np.all(np.nan_to_num(psf_dataframe_list_of_frame - loaded_data) < 1e-6))

    def test_psf_detection_preview_log_video_BOTH(self):
        psf_dataframe_list_of_frame = psf_detection_preview(self.test_obj, 'log', frame_number=[0], mode='BOTH')
        psf_dataframe_one_frame = psf_detection_preview(self.test_obj, 'log', frame_number=0, mode='BOTH')
        self.assertTrue(psf_dataframe_list_of_frame.shape == psf_dataframe_one_frame.shape)
        self.assertTrue(psf_dataframe_list_of_frame.equals(psf_dataframe_list_of_frame))
        loaded_data = load_fixture(os.path.join(self.directory_path, 'test_psf_detection_preview_log_video_both.pck'))
        self.assertTrue(np.all((psf_dataframe_one_frame - loaded_data) < 1e-6))
        self.assertTrue(np.all(np.nan_to_num(psf_dataframe_list_of_frame - loaded_data) < 1e-6))

    def test_psf_detection_preview_log_video_Bright(self):
        psf_dataframe_list_of_frame = psf_detection_preview(self.test_obj, 'log', frame_number=[0], mode='Bright')
        psf_dataframe_one_frame = psf_detection_preview(self.test_obj, 'log', frame_number=0, mode='Bright')
        self.assertTrue(psf_dataframe_list_of_frame.shape == psf_dataframe_one_frame.shape)
        self.assertTrue(psf_dataframe_list_of_frame.equals(psf_dataframe_list_of_frame))
        loaded_data = load_fixture(os.path.join(self.directory_path, 'test_psf_detection_preview_log_video_Bright.pck'))
        self.assertTrue(np.all((psf_dataframe_one_frame - loaded_data) < 1e-6))
        self.assertTrue(np.all(np.nan_to_num(psf_dataframe_list_of_frame - loaded_data) < 1e-6))

    def test_psf_detection_preview_log_video_Dark(self):
        psf_dataframe_list_of_frame = psf_detection_preview(self.test_obj, 'log', frame_number=[0], mode='Dark')
        psf_dataframe_one_frame = psf_detection_preview(self.test_obj, 'log', frame_number=0, mode='Dark')
        self.assertTrue(psf_dataframe_list_of_frame.shape == psf_dataframe_one_frame.shape)
        self.assertTrue(psf_dataframe_list_of_frame.equals(psf_dataframe_list_of_frame))
        loaded_data = load_fixture(os.path.join(self.directory_path, 'test_psf_detection_preview_log_video_Dark.pck'))
        self.assertTrue(np.all((psf_dataframe_one_frame - loaded_data) < 1e-6))
        self.assertTrue(np.all(np.nan_to_num(psf_dataframe_list_of_frame - loaded_data) < 1e-6))

    def test_psf_detection_preview_frst_one_psf_video(self):
        psf_dataframe_list_of_frame = psf_detection_preview(self.test_obj, 'frst_one_psf', frame_number=[0],
                                                            mode='BOTH')
        psf_dataframe_one_frame = psf_detection_preview(self.test_obj, 'frst_one_psf', frame_number=0, mode='BOTH')
        self.assertTrue(psf_dataframe_list_of_frame.shape == psf_dataframe_one_frame.shape)
        self.assertTrue(psf_dataframe_list_of_frame.equals(psf_dataframe_list_of_frame))
        loaded_data = load_fixture(os.path.join(self.directory_path, 'test_psf_detection_preview_frst_one_psf_video.pck'))
        self.assertTrue(np.all((psf_dataframe_one_frame - loaded_data) < 1e-6))
        self.assertTrue(np.all(np.nan_to_num(psf_dataframe_list_of_frame - loaded_data) < 1e-6))
        ...
        self.assertTrue(psf_dataframe_list_of_frame.shape == psf_dataframe_one_frame.shape)
        self.assertTrue(psf_dataframe_list_of_frame.equals(psf_dataframe_list_of_frame))
        loaded_data = load_fixture(os.path.join(self.directory_path,
                                                'test_psf_detection_preview_frst_one_psf_repeat_video.pck'))
        self.assertTrue(np.all((psf_dataframe_one_frame - loaded_data) < 1e-6))
        self.assertTrue(np.all(np.nan_to_num(psf_dataframe_list_of_frame - loaded_data) < 1e-6))

    def test_psf_detection_preview_RVT_video(self):
        psf_dataframe_list_of_frame = psf_detection_preview(self.test_obj_doh, 'RVT', frame_number=[0], mode='BOTH')
        psf_dataframe_one_frame = psf_detection_preview(self.test_obj_doh, 'RVT', frame_number=0, mode='BOTH')
        self.assertTrue(psf_dataframe_list_of_frame.shape == psf_dataframe_one_frame.shape)
        self.assertTrue(psf_dataframe_list_of_frame.equals(psf_dataframe_list_of_frame))
        loaded_data = load_fixture(os.path.join(self.directory_path, 'test_psf_detection_preview_RVT_video.pck'))
        self.assertTrue(np.all((psf_dataframe_one_frame - loaded_data) < 1e-6))
        self.assertTrue(np.all(np.nan_to_num(psf_dataframe_list_of_frame - loaded_data) < 1e-6))
    ...
    def test_psf_detection_dog_video_BOTH(self):
        ...
        loaded_data = load_fixture(os.path.join(self.directory_path, 'test_psf_detection_dog_video_both.pck'))
    ...
    def test_psf_detection_dog_video_Bright(self):
        ...
        loaded_data = load_fixture(os.path.join(self.directory_path, 'test_psf_detection_dog_video_Bright.pck'))
    ...
    def test_psf_detection_dog_video_Dark(self):
        ...
        loaded_data = load_fixture(os.path.join(self.directory_path, 'test_psf_detection_dog_video_Dark.pck'))
        self.assertTrue(np.all((psf_dataframe - loaded_data) < 1e-6))
    ...
    def test_fit_Gaussian2D_wrapper(self):
        ...
        self.test_obj.cpu.parallel_active = False
        psf_dataframe_gaussian = self.test_obj.fit_Gaussian2D_wrapper(PSF_List=psf_dataframe_p, scale=5,
                                                                      internal_parallel_flag=True)
        self.assertTrue(psf_dataframe_p.shape[0] == psf_dataframe_gaussian_p.shape[0])
    ...
    def test_difference_of_gaussian(self):
        min_range, max_range = difference_of_gaussian.dog_preview(self.video, min_sigma=1.6, max_sigma=1.8,
                                                                  sigma_ratio=1.1)
